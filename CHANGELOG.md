...
- Tests covering the connector, its device-less step list, field normalisation, validate parsing (including a non-fatal portal-lookup outage), the test-mode demo path, the seeder row, and the SSRF guard. They live in the **`regression`** suite — see Changed below.

### Changed

- **Moved the Dominion/PowerPortal connector tests from `unit` into `regression`, so CI actually enforces them.** CI runs `--testsuite=unit` with `continue-on-error: true` (documented as non-blocking until the analytics `$wpdb`-mock debt is repaired), so the 38 tests proving the connector works were passing into a suite that could not fail the build — green CI would not have meant the connector was sound. `regression` is part of the blocking Net 6 gate. Adapted to that suite's conventions (`ISF\Tests\Regression`, PHPUnit's `TestCase` with explicit Brain Monkey setup) rather than the heavier `ISF\Tests\Unit\TestCase`, which these tests never needed. No test logic changed.
